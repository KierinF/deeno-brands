...
export async function GET(request: Request) {
  ...
  const [
    ...
    supabase.from('contacts').select('*').eq('parcel_id', parcel_id).order('confidence', { ascending: false }).limit(10),
    ...
    supabase.from('signals').select('signal_type, signal_date').eq('parcel_id', parcel_id).order('signal_date', { ascending: false }).limit(5),
    supabase.from('leads').select('*').eq('parcel_id', parcel_id).maybeSingle(),
  ])

  return NextResponse.json({ building, contacts, phoneNumbers, activityLog, buildingNotes, tasks, signals, lead })
}
